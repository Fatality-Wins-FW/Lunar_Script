...
local function CreateESPObject(plr)
    ...
    local cornerLines = {}
    if hasDrawing then for i=1,8 do local l=Drawing.new("Line"); l.Thickness=1.5; l.Visible=false; table.insert(cornerLines,l) end end
    
    ESPObjects[plr] = {
        Box = Instance.new("BillboardGui", plr.Character), CornerLines = cornerLines,
        Tracer = hasDrawing and Drawing.new("Line"), Name = hasDrawing and Drawing.new("Text"),
        HealthBarBg = hasDrawing and Drawing.new("Square"), HealthBar = hasDrawing and Drawing.new("Square"), Skeletons = {}
    }
    local obj = ESPObjects[plr]
    obj.Box.Name = "LunarEsp"; obj.Box.AlwaysOnTop = true
    ...
    if hasDrawing then
        obj.Tracer.Thickness = 1; obj.Name.Size = 13; obj.Name.Center = true; obj.Name.Outline = true
        obj.HealthBarBg.Filled = true; obj.HealthBarBg.Color = Color3.fromRGB(10,10,10)
        obj.HealthBar.Filled = true; obj.HealthBar.Color = Color3.fromRGB(40,220,90)
        for i=1,15 do local l=Drawing.new("Line"); l.Thickness=1.5; table.insert(obj.Skeletons,l) end
    end
end

local function HideESPObject(plr)
    local obj = ESPObjects[plr]; if not obj then return end
...
local function createEsp(player)
    ...
    local hum = char:FindFirstChildOfClass("Humanoid")
    if not (char and root and head and hum and hum.Health > 0) then HideESPObject(player); return end
    
    local rootPos, onScreen = Camera:WorldToViewportPoint(root.Position)
    if not (onScreen and rootPos.Z > 0) then HideESPObject(player); return end
    ...
    if getgenv().LunarState.ESP then
        ...
        if style == "Full Box" then
            ...
            local function addFrame(pos, size)
                ...
                fr.BackgroundColor3 = color; fr.BorderSizePixel = 0
            end
            addFrame(UDim2.new(0,0,0,0), UDim2.new(1,0,t,0)); addFrame(UDim2.new(0,0,1-t,0), UDim2.new(1,0,t,0))
            addFrame(UDim2.new(0,0,0,0), UDim2.new(t,0,1,0)); addFrame(UDim2.new(1-t,0,0,0), UDim2.new(t,0,1,0))
        elseif style == "Corner Box" then
            local lineLen = math.clamp(width * 0.25, 4, 15)
            local w = lineLen / width; local h = lineLen / height
            ...
            addCorner(UDim2.new(0,0,0,0), UDim2.new(w,0,h,0)); addCorner(UDim2.new(1-w,0,0,0), UDim2.new(w,0,h,0))
            addCorner(UDim2.new(0,0,1-h,0), UDim2.new(w,0,h,0)); addCorner(UDim2.new(1-w,0,1-h,0), UDim2.new(w,0,h,0))
        end
    end
    ...
    if hasDrawing then
        ...
        if getgenv().LunarState.Names or getgenv().LunarState.Distance then
            ...
            obj.Name.Text = text; obj.Name.Position = Vector2.new(rootPos.X, topLeft.Y - 16)
        ...
        if getgenv().LunarState.HealthBars then
            ...
            obj.HealthBarBg.Size = Vector2.new(3, height); obj.HealthBarBg.Position = Vector2.new(topLeft.X-6, topLeft.Y); obj.HealthBarBg.Visible = true
            obj.HealthBar.Size = Vector2.new(3, height*hpPercent); obj.HealthBar.Position = Vector2.new(topLeft.X-6, topLeft.Y+(height*(1-hpPercent))); obj.HealthBar.Visible = true
...
MiscGroup:AddSlider("JumpPower", { Text="Jump Power", Min=50, Max=350, Default=50, Rounding=0, Callback=function(v) getgenv().LunarState.Config.JumpPowerVal=v end })
...
ExtraGroup:AddToggle("AntiAim", { Text="Anti Aim", Callback=function(s) getgenv().LunarState.AntiAim=s end })
...
ConfigGroup:AddButton("Save Config", function() pcall(function() writefile("LunarUniversal_V2.json", HttpService:JSONEncode(getgenv().LunarState.Config)); Library:Notify("Saved!") end) end)
ConfigGroup:AddButton("Load Config", function() pcall(function() local d=readfile("LunarUniversal_V2.json"); local dec=HttpService:JSONDecode(d); for k,v in pairs(dec) do getgenv().LunarState.Config[k]=v end; Library:Notify("Loaded!") end) end)
ConfigGroup:AddButton("Unload", function() getgenv().LunarRunning=false; for _,l in pairs(getgenv().LunarState.Lines) do l:Remove() end; if fovCircle then fovCircle:Remove() end; if MyUI then MyUI:Destroy() end; Library:Unload() end)
        ...
        if getgenv().LunarState.AntiAim or getgenv().LunarState.SpinBot then
            if not aaConn then
                aaConn = RunService.RenderStepped:Connect(function()
                    ...
                    if hrp then
                        if getgenv().LunarState.SpinBot then hrp.CFrame = hrp.CFrame * CFrame.Angles(0, math.rad(tick()*500), 0) end
                        if getgenv().LunarState.AntiAim then hrp.CFrame = hrp.CFrame * CFrame.Angles(math.rad(90), 0, 0) end
                    end
                end)
            end
        elseif aaConn then aaConn:Disconnect(); aaConn=nil end
