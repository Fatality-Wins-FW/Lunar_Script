...
function Library:Init(config)
    ...
    self.ScreenGui = Create("ScreenGui", {
        Name = "LunarUI_" .. tostring(math.random(1000, 9999)),
    ...
    self.DragHandle = Create("Frame", {
        Size = UDim2.new(1, 0, 0, 30),
        BackgroundColor3 = Color3.fromRGB(18, 16, 16),
        BorderSizePixel = 0,
        ZIndex = 10,
    ...
    self.TopBarContent = Create("Frame", {
        Size = UDim2.new(1, 0, 0, 30),
    ...
    Create("TextLabel", {
        ...
        BackgroundTransparency = 1, Text = self.Config.Title,
        TextColor3 = self.Config.AccentColor, TextSize = 16,
        Font = Enum.Font.GothamBold, TextXAlignment = Enum.TextXAlignment.Left,
        Parent = self.TopBarContent,
    })

    Create("TextLabel", {
        Size = UDim2.new(0.5, 0, 1, 0), Position = UDim2.new(0.5, 0, 0, 0),
        BackgroundTransparency = 1, Text = self.Config.Subtitle,
    ...
    Create("TextLabel", {
        Size = UDim2.new(1, -160, 0, 20), Position = UDim2.new(0, 160, 1, -20),
        BackgroundColor3 = Color3.fromRGB(18, 16, 16), BorderSizePixel = 0,
        Text = self.Config.Footer, TextColor3 = Color3.fromRGB(80, 80, 80),
        TextSize = 10, Font = Enum.Font.Gotham, Parent = self.MainFrame,
    })
...
function Library:Notify(title, message, duration)
    ...
    local notif = Create("Frame", {
        ...
        BackgroundColor3 = self.Config.BackgroundColor,
        BorderColor3 = self.Config.AccentColor,
        BorderSizePixel = 2,
    ...
    Create("TextLabel", {
        ...
        BackgroundTransparency = 1, Text = title, TextColor3 = self.Config.AccentColor,
        TextSize = 14, Font = Enum.Font.GothamBold, TextXAlignment = Enum.TextXAlignment.Left,
        Parent = notif,
    })
    
    local msgLabel = Create("TextLabel", {
        Size = UDim2.new(1, -20, 1, -30), Position = UDim2.new(0, 10, 0, 25),
        BackgroundTransparency = 1, Text = message, TextColor3 = self.Config.TextColor,
...
function Library:AddTab(name)
    local btn = Create("TextButton", {
        ...
        Position = UDim2.new(0, 6, 0, (#self.Tabs * 40) + 6),
        BackgroundColor3 = Color3.fromRGB(30, 28, 28),
    ...
    if #self.Tabs == 1 then btn:Fire() end
    return data
end
...
function Library:AddSection(tab, name)
    ...
    Create("TextLabel", {
        ...
        Text = name, TextColor3 = self.Config.AccentColor,
    ...
    local inner = Create("Frame", {
        ...
        BackgroundTransparency = 1, Parent = sec,
    })
...
function Library:AddButton(sec, cfg)
    ...
    local btn = Create("TextButton", {
        ...
        BorderSizePixel = 1, Text = cfg.Name or "Button",
        TextColor3 = self.Config.TextColor, TextSize = 12,
        Font = Enum.Font.GothamMedium, AutoButtonColor = false,
        Parent = sec.Inner,
    })
    btn.Position = UDim2.new(0, 5, 0, sec.Y)
    sec.Y = sec.Y + 38
...
function Library:AddToggle(sec, cfg)
    ...
    local cont = Create("Frame", {
        Size = UDim2.new(1, -10, 0, 32),
        BackgroundTransparency = 1, Parent = sec.Inner,
    })
    cont.Position = UDim2.new(0, 5, 0, sec.Y)
    sec.Y = sec.Y + 38
    
    Create("TextLabel", {
        Size = UDim2.new(1, -45, 1, 0), BackgroundTransparency = 1,
        Text = cfg.Name or "Toggle", TextColor3 = self.Config.TextColor,
    ...
    local box = Create("Frame", {
        ...
        BackgroundColor3 = Color3.fromRGB(45, 43, 43),
        BorderColor3 = Color3.fromRGB(70, 68, 68),
        BorderSizePixel = 1, Parent = cont,
    })
    ...
    local ind = Create("Frame", {
        ...
        BackgroundColor3 = self.Config.AccentColor,
        BorderSizePixel = 0, Visible = cfg.Default or false,
        Parent = box,
    })
    
    local val = cfg.Default or false
    self.Flags[cfg.Flag or cfg.Name] = val
    ...
    return {SetValue = function(_, v) 
        val = v; ind.Visible = v; self.Flags[cfg.Flag or cfg.Name] = v
...
function Library:AddSlider(sec, cfg)
    ...
    Create("TextLabel", {
        Size = UDim2.new(1, -50, 0, 15), BackgroundTransparency = 1,
        Text = cfg.Name or "Slider", TextColor3 = self.Config.TextColor,
        TextSize = 12, Font = Enum.Font.GothamMedium,
        TextXAlignment = Enum.TextXAlignment.Left, Parent = cont,
    })
    
    local valLabel = Create("TextLabel", {
        Size = UDim2.new(0, 45, 0, 15), Position = UDim2.new(1, -45, 0, 0),
        BackgroundTransparency = 1, Text = tostring(def),
        TextColor3 = self.Config.AccentColor, TextSize = 12,
    ...
    local cur = def
    self.Flags[cfg.Flag or cfg.Name] = cur
    ...
    UserInputService.InputChanged:Connect(function(i)
        if drag and i.UserInputType == Enum.UserInputType.MouseMovement then
            ...
            nv = Round(nv, dec)
            cur = nv
            self.Flags[cfg.Flag or cfg.Name] = nv
    ...
    self:_UpdateCanvas(sec.ParentTab.Cont)
    return {SetValue = function(_, v)
        v = math.clamp(v, min, max); cur = v; self.Flags[cfg.Flag or cfg.Name] = v
        fill.Size = UDim2.new((v - min) / (max - min), 0, 1, 0); valLabel.Text = tostring(v)
        if cfg.Callback then cfg.Callback(v) end
    end}
end
...
function Library:AddDropdown(sec, cfg)
    ...
    Create("TextLabel", {
        ...
        Text = cfg.Name or "Dropdown", TextColor3 = self.Config.TextColor,
        TextSize = 12, Font = Enum.Font.GothamMedium,
        TextXAlignment = Enum.TextXAlignment.Left, Parent = cont,
    })
    ...
    local box = Create("TextButton", {
        ...
        BackgroundColor3 = Color3.fromRGB(38, 36, 36),
        BorderColor3 = Color3.fromRGB(55, 53, 53),
        BorderSizePixel = 1, Text = def or "",
        TextColor3 = self.Config.TextColor, TextSize = 11,
        Font = Enum.Font.GothamMedium, AutoButtonColor = false,
        Parent = cont,
    })
    
    local list = Create("Frame", {
        Size = UDim2.new(0, 90, 0, 0), Position = UDim2.new(1, -95, 1, 2),
        BackgroundColor3 = self.Config.SecondaryColor,
        BorderColor3 = self.Config.AccentColor,
        BorderSizePixel = 1, ClipsDescendants = true,
        Visible = false, ZIndex = 10, Parent = cont,
    })
    ...
    local function refresh()
        ...
        for _, o in ipairs(opts) do
            local b = Create("TextButton", {
                Size = UDim2.new(1, 0, 0, 24), Position = UDim2.new(0, 0, 0, y),
                BackgroundColor3 = Color3.fromRGB(38, 36, 36),
                BorderSizePixel = 0, Text = o,
                TextColor3 = self.Config.TextColor, TextSize = 11,
                Font = Enum.Font.GothamMedium, AutoButtonColor = false,
                Parent = list,
            })
            b.MouseButton1Click:Connect(function()
                cur = o; self.Flags[cfg.Flag or cfg.Name] = o
                box.Text = o; if cfg.Callback then cfg.Callback(o) end
                open = false; list.Visible = false; list.Size = UDim2.new(0, 90, 0, 0)
            end)
            y = y + 24
        end
    ...
    box.MouseButton1Click:Connect(function()
        ...
        if open then refresh() else list.Size = UDim2.new(0, 90, 0, 0) end
    end)
    
    self:_UpdateCanvas(sec.ParentTab.Cont)
    return {SetValue = function(_, v) 
        cur = v; box.Text = v; self.Flags[cfg.Flag or cfg.Name] = v
        if cfg.Callback then cfg.Callback(v) end 
    end}
end
...
function Library:AddColorPicker(sec, cfg)
    ...
    Create("TextLabel", {
        Size = UDim2.new(1, -45, 1, 0), BackgroundTransparency = 1,
        Text = cfg.Name or "Color", TextColor3 = self.Config.TextColor,
        TextSize = 12, Font = Enum.Font.GothamMedium,
        TextXAlignment = Enum.TextXAlignment.Left, Parent = cont,
    })
    ...
    local box = Create("Frame", {
        Size = UDim2.new(0, 34, 0, 18),
        Position = UDim2.new(1, -40, 0.5, -9),
        BackgroundColor3 = cfg.Default or Color3.new(1, 1, 1),
        BorderColor3 = Color3.fromRGB(70, 68, 68),
        BorderSizePixel = 1, Parent = cont,
    })
    
    local cur = cfg.Default or Color3.new(1, 1, 1)
    self.Flags[cfg.Flag or cfg.Name] = cur
    if cfg.Callback then cfg.Callback(cur) end
    ...
    box.InputBegan:Connect(function(i)
        if i.UserInputType == Enum.UserInputType.MouseButton1 then
            local hues = {
                ...
                Color3.fromRGB(60, 60, 255), Color3.fromRGB(255, 255, 60),
                Color3.fromRGB(255, 60, 255), Color3.fromRGB(60, 255, 255)
            }
            local idx = 1
            for x, h in ipairs(hues) do if h == cur then idx = x break end end
            cur = hues[(idx % #hues) + 1]
    ...
    self:_UpdateCanvas(sec.ParentTab.Cont)
    return {SetValue = function(_, c) 
        cur = c; box.BackgroundColor3 = c; self.Flags[cfg.Flag or cfg.Name] = c
...
function Library:AddKeybind(sec, cfg)
    ...
    Create("TextLabel", {
        ...
        Text = cfg.Name or "Keybind", TextColor3 = self.Config.TextColor,
        TextSize = 12, Font = Enum.Font.GothamMedium,
        TextXAlignment = Enum.TextXAlignment.Left, Parent = cont,
    })
    
    local box = Create("TextButton", {
        Size = UDim2.new(0, 70, 0, 24), Position = UDim2.new(1, -75, 0.5, -12),
        BackgroundColor3 = Color3.fromRGB(38, 36, 36),
        BorderColor3 = Color3.fromRGB(55, 53, 53),
        BorderSizePixel = 1, Text = (cfg.Default or Enum.KeyCode.F).Name,
        TextColor3 = self.Config.TextColor, TextSize = 11,
        Font = Enum.Font.GothamMedium, AutoButtonColor = false,
        Parent = cont,
    })
    
    local cur = cfg.Default or Enum.KeyCode.F
    self.Flags[cfg.Flag or cfg.Name] = cur
    local wait = false
    ...
    UserInputService.InputBegan:Connect(function(i, gp)
        if wait and i.UserInputType == Enum.UserInputType.Keyboard then
            wait = false; cur = i.KeyCode; box.Text = cur.Name
            self.Flags[cfg.Flag or cfg.Name] = cur
            if cfg.Callback then cfg.Callback(cur) end
        end
    end)
    ...
    self:_UpdateCanvas(sec.ParentTab.Cont)
    return {SetValue = function(_, k) 
        cur = k; box.Text = k.Name; self.Flags[cfg.Flag or cfg.Name] = k
...
function Library:AddTextbox(sec, cfg)
    ...
    Create("TextLabel", {
        ...
        Text = cfg.Name or "Textbox", TextColor3 = self.Config.TextColor,
        TextSize = 12, Font = Enum.Font.GothamMedium,
        TextXAlignment = Enum.TextXAlignment.Left, Parent = cont,
    })
    
    local tb = Create("TextBox", {
        Size = UDim2.new(1, -10, 0, 26), Position = UDim2.new(0, 5, 0, 22),
        BackgroundColor3 = Color3.fromRGB(38, 36, 36),
        BorderColor3 = Color3.fromRGB(55, 53, 53),
        BorderSizePixel = 1, Text = cfg.Default or "",
    ...
    local cur = cfg.Default or ""
    self.Flags[cfg.Flag or cfg.Name] = cur
    if cfg.Callback then cfg.Callback(cur) end
    
    tb.FocusLost:Connect(function()
        cur = tb.Text; self.Flags[cfg.Flag or cfg.Name] = cur
        if cfg.Callback then cfg.Callback(cur) end
    end)
    
    self:_UpdateCanvas(sec.ParentTab.Cont)
    return {SetValue = function(_, t) 
        cur = t; tb.Text = t; self.Flags[cfg.Flag or cfg.Name] = t
        if cfg.Callback then cfg.Callback(t) end 
    end}
end
...
table.insert(Library.UnloadFunctions, function() 
...
function Library:_SetupDragResize()
    ...
    UserInputService.InputChanged:Connect(function(i)
        if dragging and i.UserInputType == Enum.UserInputType.MouseMovement then
            local newPos = i.Position - dragOffset
            self.MainFrame.Position = UDim2.new(0, newPos.X, 0, newPos.Y)
        end
        if resizing and i.UserInputType == Enum.UserInputType.MouseMovement then
            local delta = i.Position - resizeStartPos
            local newSize = Vector2.new(
                math.clamp(resizeStartSize.X + delta.X, 600, 1200),
                math.clamp(resizeStartSize.Y + delta.Y, 400, 800)
            )
...
function Library:_SetupMenuToggle()
    UserInputService.InputBegan:Connect(function(i, gp)
        if i.UserInputType == Enum.UserInputType.Keyboard and i.KeyCode == self.MenuKey then
